...
// Generate a unique token for this form instance if one doesn't exist
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Check if this is a valid form submission with the correct token
    if (isset($_POST['form_token']) && $_POST['form_token'] === $_SESSION['form_token']) {
        ...
        error_log("Registration form submitted: " . json_encode($_POST));
            ...
            if ($_FILES['photo']['size'] > 2 * 1024 * 1024) {
                ...
                if (in_array(strtolower($fileType), $allowedTypes)) {
                    if (move_uploaded_file($_FILES['photo']['tmp_name'], $targetFilePath)) {
                        $_POST['profilePicturePath'] = $targetFilePath;
                    } else {
        ...
        // Only proceed if there are no errors with file upload
        if (!isset($errMsg)) {
            // Let the controller handle the registration process
            if ($authController->processRegistration($_POST)) {
                // Generate a new token to prevent resubmission
                $_SESSION['form_token'] = bin2hex(random_bytes(32));
                
                $_SESSION['registration_success'] = "Registration successful!";
                $_SESSION['user_role'] = $_POST['role']; // Store the user's role in the session
                header("Location: registration-success.php");
                exit();
            } else {
                // Get error message from session if available
                $errMsg = $_SESSION['errMsg'] ?? "Failed to register. Please try again.";
            }
        }
    } else {
        // This is a resubmission or invalid token, redirect to prevent refresh issues
