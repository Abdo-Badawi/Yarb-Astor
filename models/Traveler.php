...
class Traveler extends User {
    ...
    // Traveler-specific getters and setters
    ...
    // Override the parent method to insert traveler-specific data
    ...
    public function getTravelerProfile($traveler_id) {
        $query = "SELECT u.*, t.* 
                  FROM users u 
                  JOIN traveler t ON u.user_id = t.traveler_id 
                  WHERE u.user_id = ?";
        $result = $this->db->selectPrepared($query, "i", [$traveler_id]);
    ...
    public function updateTravelerProfile() {
        if (!$this->db->openConnection()) {
            return false;
        }
        ...
        try {
            ...
            if (!$travelerResult) {
                throw new Exception("Failed to update traveler information.");
            }
